import React, { Component } from "react";
import SubHeader from "../../components/common/sub-header";

class Government extends Component {
  constructor(props, context) {
    super(props, context);
  }

  componentDidMount() {
    window.scrollTo(0, 0);
  }

  render() {
    return (
      <div>
        <SubHeader title="Government Solutions" />

        <div className="content-block  stick-to-footer">
          <div className="page-container container">
            <div className="row">
              <div className="col-md-12 entry-content">
                <div className="vc_row wpb_row vc_row-fluid vc_custom_1493982176749">
                  <div className="wpb_column vc_column_container vc_col-sm-12">
                    <div className="vc_column-inner ">
                      <div className="wpb_wrapper">
                        <div
                          className="vc_tta-container"
                          data-vc-action="collapse"
                        >
                          <div className="vc_general vc_tta vc_tta-tabs vc_tta-color-grey vc_tta-style-outline vc_tta-shape-rounded vc_tta-o-shape-group vc_tta-gap-10 vc_tta-o-no-fill vc_tta-tabs-position-top vc_tta-controls-align-center">
                            <div className="vc_tta-tabs-container">
                              <ul className="vc_tta-tabs-list">
                                <li
                                  className="vc_tta-tab vc_active"
                                  data-vc-tab
                                >
                                  <a
                                    href="#biometrics"
                                    data-vc-tabs
                                    data-vc-container=".vc_tta"
                                  >
                                    <span className="vc_tta-title-text">
                                      National ID Card
                                    </span>
                                  </a>
                                </li>
                                <li className="vc_tta-tab" data-vc-tab>
                                  <a
                                    href="#identity-solutions"
                                    data-vc-tabs
                                    data-vc-container=".vc_tta"
                                  >
                                    <span className="vc_tta-title-text">
                                      International Passports
                                    </span>
                                  </a>
                                </li>

                                <li className="vc_tta-tab" data-vc-tab>
                                  <a
                                    href="#telecommunications"
                                    data-vc-tabs
                                    data-vc-container=".vc_tta"
                                  >
                                    <span className="vc_tta-title-text">
                                      Voters/Election Card
                                    </span>
                                  </a>
                                </li>

                                <li className="vc_tta-tab" data-vc-tab>
                                  <a
                                    href="#portfolio"
                                    data-vc-tabs
                                    data-vc-container=".vc_tta"
                                  >
                                    <span className="vc_tta-title-text">
                                      Immigration
                                    </span>
                                  </a>
                                </li>
                                <li className="vc_tta-tab" data-vc-tab>
                                  <a
                                    href="#identity-solution"
                                    data-vc-tabs
                                    data-vc-container=".vc_tta"
                                  >
                                    <span className="vc_tta-title-text">
                                      Others
                                    </span>
                                  </a>
                                </li>
                              </ul>
                            </div>
                            <div className="vc_tta-panels-container">
                              <div className="vc_tta-panels">
                                <div
                                  className="vc_tta-panel vc_active"
                                  id="biometrics"
                                  data-vc-content=".vc_tta-panel-body"
                                >
                                  <div className="vc_tta-panel-heading">
                                    <h4 className="vc_tta-panel-title">
                                      <a
                                        href="#biometrics"
                                        data-vc-accordion
                                        data-vc-container=".vc_tta-container"
                                      >
                                        <span className="vc_tta-title-text">
                                          Prototypes
                                        </span>
                                      </a>
                                    </h4>
                                  </div>
                                  <div className="vc_tta-panel-body">
                                    <div className="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1487333607675">
                                      <div className="wpb_column vc_column_container vc_col-sm-8">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-header-block clearfix text-left text-black wpb_animate_when_almost_visible wpb_fadeInLeft fadeInLeft wpb_content_element  mgt-header-block-style-2 mgt-header-block-fontsize-medium mgt-header-texttransform-none mgt-header-block-3957370  vc_custom_1496224728514">
                                              <h2 className="mgt-header-block-title text-font-weight-default">
                                                National ID Card
                                              </h2>
                                            </div>
                                            <div className="wpb_text_column wpb_content_element  wpb_animate_when_almost_visible wpb_fadeInLeft fadeInLeft text-size-medium">
                                              <div className="wpb_wrapper">
                                                <p>
                                                  As Africa's leading
                                                  polycarbonate smartcard
                                                  manufacturing company,
                                                  SecureID possesses the
                                                  exquisite machinery and
                                                  materials to manufacture
                                                  national ID cards with
                                                  improved security that
                                                  extinguishes ID fraud and
                                                  identity theft. Our national
                                                  ID cards boast of greater
                                                  security and high security
                                                  printing features to deter
                                                  fraud. Our use of
                                                  polycarbonate guarantees an
                                                  impregnable wall of security
                                                  and enhanced level of ID
                                                  document security. Its layered
                                                  structure suits the deployment
                                                  of security and multipurpose
                                                  features
                                                </p>
                                              
                                                <br/>
                                                <span>
                                                <strong>SOME SECURITY FEATURE</strong>

                                                <ul>
                                                <li>Visual security features which includes holographic over laminates and card pre-printing.</li>
                                                <li>Holographic over laminates may include overt, covert, and forensic features.</li>
                                                <li>Pre-printing options range from holograms and various types of security printing to special inks, relief structures, laser images, optical variable ink (OVI), hidden features, randomised UV markings etc.</li>

                                                </ul>
                                                </span>

                                                <br/>
                                                <span>
                                                <strong>SOME BENEFITS</strong>

                                                <ul>
                                                  <li>National ID cards help associate a lot of information with a particular person.</li>
                                                  <li>National ID cards are implemented for national security reasons</li>
                                                  <li>National ID cards are solutions to help the government combat against identity thefts</li>
                                                  <li>National ID helps in ease of administering social incentives to populace</li>
                                                  <li>Our cards can be used to fight against illegal employment through staff verification.  </li>
                                                  

                                                </ul>
                                                </span>
                                                </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>

                                      <div className="wpb_column vc_column_container vc_col-sm-4">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-promo-block-container wpb_content_element wpb_animate_when_almost_visible wpb_fadeInRight fadeInRight">
                                              <div className="mgt-promo-block-wrapper  mgt-promo-block-shadow">
                                                <div
                                                  className=" mgt-promo-block animated black-text text-color-hover-white cover-image no-darken mgt-promo-block-74695761"
                                                  data-style="background-color: #f5f5f5;background-image: url(assets/img/scm/national-card.PNG);background-repeat: no-repeat;height: 350px;"
                                                >
                                                  <div className="mgt-promo-block-content mgt-hide-button va-middle">
                                                    <div className="mgt-promo-block-content-inside mgt-promo-block-content-inside-show-on-hover" />
                                                  </div>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div
                                  className="vc_tta-panel"
                                  id="identity-solutions"
                                  data-vc-content=".vc_tta-panel-body"
                                >
                                  <div className="vc_tta-panel-heading">
                                    <h4 className="vc_tta-panel-title">
                                      <a
                                        href="#identity-solutions"
                                        data-vc-accordion
                                        data-vc-container=".vc_tta-container"
                                      >
                                        <span className="vc_tta-title-text">
                                          Development
                                        </span>
                                      </a>
                                    </h4>
                                  </div>
                                  <div className="vc_tta-panel-body">
                                    <div className="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1487333607675">
                                      <div className="wpb_column vc_column_container vc_col-sm-8">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-header-block clearfix text-left text-black wpb_content_element  mgt-header-block-style-2 mgt-header-block-fontsize-medium mgt-header-texttransform-none mgt-header-block-58442946  vc_custom_1487761220473">
                                              <h2 className="mgt-header-block-title text-font-weight-default">
                                                International Passports
                                              </h2>
                                            </div>
                                            <div className="wpb_text_column wpb_content_element  text-size-medium">
                                              <div className="wpb_wrapper">
                                                <p>
                                                  In partnership with world
                                                  industry leaders in passport
                                                  design, passport printing and
                                                  booklet assembly
                                                  manufacturers, SecureID's
                                                  passport offer solutions for
                                                  basic and advanced passports
                                                  featuring:
                                                </p>

                                                <span>
                                                <ul>
                                                  <li>Varied Data page positions</li>
                                                  <li>Varied position of chip</li>
                                                  <li>RF Shield for privacy</li>
                                                </ul>
                                                </span>

                                                <br />
                                                <span>
                                                  <strong>SOME SECURITY FEATURE</strong>

                                                  <br />
                                                  Our printing technologies offer offset printing as well as intaglio printing. The security features in offset and intaglio printing include but are not limited to:

                                                  <ul>
                                                    <li>Offset: Rainbow, Micro text, adjacent lines, Fluorescent inks, IR Pairing and many more.</li>
                                                    <li>Intaglio: OVI, Latent image, Tactility and many more.</li>
                                                  </ul>

                                                  <br />
                                                  There are varied options for design using chain or interlock stitching sewing.
Our passports come in both Polycarbonate and Paper based ID page.

                                                </span>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>

                                      <div
                                        className="wpb_column vc_column_container vc_col-sm-4"
                                        style={{ visibility: "hidden" }}
                                      >
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-43183551 mgt-icon-font text-black  vc_custom_1487761270512">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-banknote" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Great Service
                                                </h5>
                                                SecureID Limited alongside its
                                                subsidiary SecureCard
                                                Manufacturing is Africa’s
                                                leading Card manufacturing
                                              </div>
                                              <div className="clearfix" />
                                            </div>

                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-78792589 mgt-icon-font text-black  vc_custom_1487761280152">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-cloud" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Fast Support
                                                </h5>SecureID Limited alongside
                                                its subsidiary SecureCard
                                                Manufacturing is Africa’s
                                                leading Card manufacturing
                                              </div>
                                              <div className="clearfix" />
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>

                                <div
                                  className="vc_tta-panel"
                                  id="telecommunications"
                                  data-vc-content=".vc_tta-panel-body"
                                >
                                  <div className="vc_tta-panel-heading">
                                    <h4 className="vc_tta-panel-title">
                                      <a
                                        href="#telecommunications"
                                        data-vc-accordion
                                        data-vc-container=".vc_tta-container"
                                      >
                                        <span className="vc_tta-title-text">
                                          Development
                                        </span>
                                      </a>
                                    </h4>
                                  </div>
                                  <div className="vc_tta-panel-body">
                                    <div className="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1487333607675">
                                      <div className="wpb_column vc_column_container vc_col-sm-8">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-header-block clearfix text-left text-black wpb_content_element  mgt-header-block-style-2 mgt-header-block-fontsize-medium mgt-header-texttransform-none mgt-header-block-58442946  vc_custom_1487761220473">
                                              <h2 className="mgt-header-block-title text-font-weight-default">
                                                Voters/ Election Card
                                              </h2>
                                            </div>
                                            <div className="wpb_text_column wpb_content_element  text-size-medium">
                                              <div className="wpb_wrapper">
                                                <p>
                                                  With the ever evolving threats
                                                  of electoral fraud, SecureID
                                                  is the answer to secure
                                                  electoral documents.
                                                  SecureID's preferred solution
                                                  of the voter's card is to
                                                  allow each bonafide voter get
                                                  a Secure voter's card encoded
                                                  with the holders' biodata,
                                                  biometrics, and voting
                                                  information. The voter's cards
                                                  are expected to be much
                                                  secured. In addition to the
                                                  security features, it should
                                                  fight against fraud and ensure
                                                  transparent elections.
                                                </p>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>

                                      <div
                                        className="wpb_column vc_column_container vc_col-sm-4"
                                        style={{ visibility: "hidden" }}
                                      >
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-49100959 mgt-icon-font text-black  vc_custom_1487701807718">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-vallet" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Innovation
                                                </h5>SecureID Limited alongside
                                                its subsidiary SecureCard
                                                Manufacturing is Africa’s
                                                leading Card manufacturing
                                              </div>
                                              <div className="clearfix" />
                                            </div>

                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-84361141 mgt-icon-font text-black  vc_custom_1487701817110">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-settings" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Card Manufacturing
                                                </h5>SecureID Limited alongside
                                                its subsidiary SecureCard
                                                Manufacturing is Africa’s
                                                leading Card manufacturing
                                              </div>
                                              <div className="clearfix" />
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>

                                <div
                                  className="vc_tta-panel"
                                  id="portfolio"
                                  data-vc-content=".vc_tta-panel-body"
                                >
                                  <div className="vc_tta-panel-heading">
                                    <h4 className="vc_tta-panel-title">
                                      <a
                                        href="#portfolio"
                                        data-vc-accordion
                                        data-vc-container=".vc_tta-container"
                                      >
                                        <span className="vc_tta-title-text">
                                          Support
                                        </span>
                                      </a>
                                    </h4>
                                  </div>
                                  <div className="vc_tta-panel-body">
                                    <div className="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1487333607675">
                                      <div className="wpb_column vc_column_container vc_col-sm-8">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-header-block clearfix text-left text-black wpb_content_element  mgt-header-block-style-2 mgt-header-block-fontsize-medium mgt-header-texttransform-none mgt-header-block-50450567  vc_custom_1487761475068">
                                              <h2 className="mgt-header-block-title text-font-weight-default">
                                                Immigration
                                              </h2>
                                            </div>
                                            <div className="wpb_text_column wpb_content_element  text-size-medium">
                                              <div className="wpb_wrapper">
                                                <p>
                                                  SecureID's innovative card
                                                  designs can provide
                                                  state-of-the-art, impossible
                                                  to replicate immigration cards
                                                  with the highest level of
                                                  security features such as the
                                                  Nigerian CERPAC card.
                                                </p>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>

                                      <div
                                        className="wpb_column vc_column_container vc_col-sm-4"
                                        style={{ visibility: "hidden" }}
                                      >
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-4975600 mgt-icon-font text-black  vc_custom_1487761270512">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-banknote" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Great Service
                                                </h5>
                                                SecureID Limited alongside its
                                                subsidiary SecureCard
                                                Manufacturing is Africa’s
                                                leading Card manufacturing
                                              </div>
                                              <div className="clearfix" />
                                            </div>

                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-55234225 mgt-icon-font text-black  vc_custom_1487761280152">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-cloud" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Fast Support
                                                </h5>SecureID Limited alongside
                                                its subsidiary SecureCard
                                                Manufacturing is Africa’s
                                                leading Card manufacturing
                                              </div>
                                              <div className="clearfix" />
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div
                                  className="vc_tta-panel"
                                  id="identity-solution"
                                  data-vc-content=".vc_tta-panel-body"
                                >
                                  <div className="vc_tta-panel-heading">
                                    <h4 className="vc_tta-panel-title">
                                      <a
                                        href="#identity-solution"
                                        data-vc-accordion
                                        data-vc-container=".vc_tta-container"
                                      >
                                        <span className="vc_tta-title-text">
                                          Design
                                        </span>
                                      </a>
                                    </h4>
                                  </div>
                                  <div className="vc_tta-panel-body">
                                    <div className="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1487333607675">
                                      <div className="wpb_column vc_column_container vc_col-sm-4">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-header-block clearfix text-left text-black wpb_content_element  mgt-header-block-style-2 mgt-header-block-fontsize-medium mgt-header-texttransform-none mgt-header-block-4458008  vc_custom_1487761300471">
                                              <h2 className="mgt-header-block-title text-font-weight-default">
                                                Others
                                              </h2>
                                            </div>
                                            <div className="wpb_text_column wpb_content_element  text-size-medium">
                                              <div className="wpb_wrapper">
                                                <p>
                                                  In furtherance to the Federal
                                                  and some State Ministry of
                                                  Transport initiative and drive
                                                  to modernize transportation,
                                                  SecureID Limited has advanced
                                                  a smartcard/contactless
                                                  Electronic Ticket Based
                                                  Automated Transport Fare
                                                  Collection and Station Access
                                                  Control solution to the
                                                  transport sector.
                                                </p>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className="wpb_column vc_column_container vc_col-sm-4">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-icon-box wpb_content_element mgt-icon-box-right mgt-icon-box-icon-size-large mgt-icon-box-8670855 mgt-icon-font text-black  vc_custom_1487701807718">
                                              <div className="mgt-icon-box-icon hvr-icon-wobble-horizontal">
                                                <i className="mgt-font-icon vc_li vc_li-vallet" />
                                              </div>
                                              <div className="mgt-icon-box-content">
                                                <h5 className="text-font-weight-bold">
                                                  Transport, Health, Military
                                                  and Other related Card
                                                </h5>
                                                <p>
                                                  <strong>
                                                    Our Solution will provide:
                                                  </strong>
                                                </p>

                                                <p>
                                                  Strong identification and
                                                  authentication for patients
                                                  and healthcare professionals,
                                                  ability to verify benefits,
                                                  expiry dates, multiple uses,
                                                  are key features of microchip
                                                  cards.
                                                </p>

                                                <p>
                                                  Creation of personalized,
                                                  online services, with "patient
                                                  centric" focus with the usage
                                                  of security features and
                                                  capabilities of health care
                                                  cards (identification number,
                                                  PIN or biometric
                                                  authentication).
                                                </p>
                                              </div>
                                              <div className="clearfix" />
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className="wpb_column vc_column_container vc_col-sm-4">
                                        <div className="vc_column-inner ">
                                          <div className="wpb_wrapper">
                                            <div className="mgt-promo-block-container wpb_content_element">
                                              <div className="mgt-promo-block-wrapper  mgt-promo-block-shadow">
                                                <div
                                                  className=" mgt-promo-block animated black-text text-color-hover-white cover-image no-darken mgt-promo-block-81008001"
                                                  data-style="background-color: #f5f5f5;background-image: url(assets/img/scm/immigration-card.PNG);background-repeat: no-repeat;height: 350px;"
                                                >
                                                  <div className="mgt-promo-block-content mgt-hide-button va-middle">
                                                    <div className="mgt-promo-block-content-inside mgt-promo-block-content-inside-show-on-hover" />
                                                  </div>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="vc_row-full-width vc_clearfix" />
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}

export default Government;
